...
module.exports = class DataBase {
  ...
  getUserByIdAcc(id) {
    ...
    return new Promise(function(resolve, reject) {
      self.connection.getConnection().then(conn => {
        conn
          .query("SELECT rank, game_id, country FROM users where IdAcc = ?", [
            id
          ])
          .then(rows => {
            conn.release();
  ...
  getGuildByIdAcc(id) {
    ...
    return new Promise(function(resolve, reject) {
      self.connection.getConnection().then(conn => {
        conn
          .query(
            "SELECT m.Id, m.Job, g.Name from guild_member m INNER JOIN guild g ON m.Id = g.Id WHERE m.UserId = ?",
            [id]
          )
          .then(rows => {
            conn.release();
  ...
  getUserAvatarsByIdAcc(id) {
    ...
    return new Promise(function(resolve, reject) {
      self.connection.getConnection().then(conn => {
        conn
          .query(
            "SELECT Id, UserId, aId, type, is_cash, is_gift, amount, expire_time FROM user_avatars WHERE UserId = ?",
            [id]
          )
          .then(rows => {
            conn.release();
      ...
      self
        ...
        .then(function() {
          ...
          if (account.player.cash < discount) {
            ...
            self
              .updateGameIdAndCashByIdAcc(nname, discount, account.user_id)
              .then(function(rows2) {
                data.change = true;
                return resolve(data);
              });
  ...
  updateGoldGpWinLossByIdAcc(win_gold, win_gp, is_win, is_loss, user_id) {
    ...
    return new Promise(function(resolve, reject) {
      self.connection.getConnection().then(conn => {
        conn
          .query(
            "UPDATE users SET gold = gold + ?, gp = gp + ?, win = win + ?, loss = loss + ? WHERE IdAcc = ?",
            [win_gold, win_gp, is_win, is_loss, user_id]
          )
          .then(rows => {
            conn.release();
            if (rows[0].affectedRows > 0 || rows[0].changedRows > 0)
  ...
  getEventLogByIdAcc(user_id) {
    ...
    return new Promise(function(resolve, reject) {
      self.connection.getConnection().then(conn => {
        conn
          .query("SELECT Event1, Event2 FROM event_log WHERE Id = ?", [user_id])
          .then(rows => {
            conn.release();
            if (rows[0].length > 0) return resolve(rows);
            else return reject();
  ...
  getAccountByFBId(id) {
    ...
    return new Promise(function(resolve, reject) {
      self.connection.getConnection().then(conn => {
        conn
          .query("SELECT Id, Session FROM accounts where facebook_id = ?", [id])
          .then(rows => {
            conn.release();
            if (rows[0].length > 0) return resolve(rows);
            else return reject();
  ...
  getUserByGameId(game_id) {
    ...
    return new Promise(function(resolve, reject) {
      self.connection.getConnection().then(conn => {
        conn
          .query(
            "SELECT IdAcc, game_id, photo_url, bg_url, country, rank, gp, win, gender, loss FROM users where LOWER(`game_id`) = ?",
            [game_id]
          )
          .then(rows => {
            conn.release();
